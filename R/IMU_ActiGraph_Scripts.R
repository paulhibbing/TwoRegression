...
## Raw data file reader function
read.AG.raw <- function(file, output.window = 1) {
    timer <- proc.time()
    
    if (sum(grepl("data.table", search())) == 0) 
        library(data.table)
    if (sum(grepl("dplyr", search())) == 0) 
        library(dplyr)
    cat("\nProcessing", file, "...")
    
    ## Get file metadata (sampling frequency and timestamps)
    file.meta <- data.frame(data.table::fread(file, nrow = 10, header = F, sep = "\n"))
    samp.freq <- file.meta[sapply(file.meta, function(x) grepl("Hz", x, ignore.case = T)), ]
    samp.freq <- as.numeric(unlist(strsplit(samp.freq, " "))[which(grepl("Hz", unlist(strsplit(samp.freq, " ")), 
        ignore.case = T)) - 1])
    
    start.time <- gsub("[[:alpha:] ]", "", file.meta[sapply(file.meta, function(x) grepl("start[. ]time", x, 
        ignore.case = T)), ])
    start.date <- gsub("[[:alpha:] ]", "", file.meta[sapply(file.meta, function(x) grepl("start[. ]date", x, 
        ignore.case = T)), ])
    start <- as.POSIXlt(paste(start.date, start.time), format = "%m/%d/%Y %H:%M:%S")
    
    ## Read file in
    data <- data.table::fread(file, stringsAsFactors = F, showProgress = F)
    data$Timestamp = start + ((seq_len(nrow(data)) - 1)/samp.freq)
    
    ## Collapse timestamps
    blocksize <- samp.freq * output.window
    Block <- rep(seq_len(floor(nrow(data)/blocksize)), each = blocksize)
    
    data$Block <- Block
    Gx = data$`Accelerometer X`
    Gy = data$`Accelerometer Y`
    Gz = data$`Accelerometer Z`  ## For ENMO in next section
    data <- data %>% group_by(Block) %>% summarise(Timestamp = first(Timestamp))
    
    ## Get ENMO
    ENMO = sqrt(Gx^2 + Gy^2 + Gz^2) - 1
    ENMO[which(ENMO < 0)] = 0
    ENMO2 = cumsum(ENMO)
    ENMO3 = diff(ENMO2[seq(1, length(ENMO), by = (samp.freq * output.window))])/(samp.freq * output.window)
    
    final_length = min(c(length(ENMO3), nrow(data)))
    data <- data[1:final_length, ]
    ENMO3 <- ENMO3[1:final_length]
    data$ENMO <- ENMO3 * 1000
    
    data <- subset(data, select = -c(grepl("block", names(data), ignore.case = T)))
    data$filesource <- file
    data$dateProcessed = Sys.time()
    
    data$dayofyear <- with(data, get.dayofyear(Timestamp, format = "%Y-%m-%d %H:%M:%S"))
    data$minofday <- with(data, get.minofday.int(Timestamp, format = "%Y-%m-%d %H:%M:%S"))
    
    order <- c("filesource", "dateProcessed", "Timestamp", "dayofyear", "minofday", "ENMO")
    data <- data[, c(order, setdiff(names(data), order))]
