...
on the AGread GitHub webpage](https://github.com/paulhibbing/AGread/issues/new) if you need extra help.

The TwoRegression package provides some sample data to show how you
might apply the models. If you can get your own data into a form that mirrors
the sample data, you'll be in good shape. Here's how to access the built-in
datasets:
...
that's not the case for the Hibbing models. Here's a look at the output from the
prior commands:
...
'participant'). The other step is calculating the coefficient of variation. We
technically could have avoided this by choosing not to delete the CV variables
earlier. But that decision now gives us an excuse to show how the calculations
work, and there are some technical reasons to take this approach as well (see
